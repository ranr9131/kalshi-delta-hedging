"""
Real-time SOL/USD price via Coinbase Exchange WebSocket. Mirrors eth_feed.py
exactly, just subscribes to SOL-USD instead of ETH-USD.
"""
...
def _on_error(ws, error):
    print(f"[sol_feed] WebSocket error: {error}")


def _on_close(ws, code, msg):
    print(f"[sol_feed] WebSocket closed: {code} {msg}")
...
if __name__ == "__main__":
    print("Starting SOL feed. Streaming for 30s...")
    start()
    for _ in range(30):
        print(f"  SOL-USD: {get_price()}  (age: {get_price_age():.1f}s)")
        time.sleep(1)
