"""
Strategies 8 & 9 on KXETH15M (30 days, executable fills, exact fee, no lookahead).

  S8  cross-asset lead-lag — buy the ETH contract side matching BTC's move
      since the ETH window opened (BTC as the signal, ETH book as the venue).
  S9  ETH continuation     — same-asset continuation taker (is the ETH book
...
ETH minute prices from eth_cache. Both lag-corrected: price at t = close of
...
CACHE = os.path.join(HERE, "eth_cache")
...
def main():
    ...
    print(f"ETH markets: {len(markets)}  eth spot: {len(eth):,}  btc spot: {len(btc):,}")
    ...
    print("\n── S9: ETH same-asset continuation taker")
    ...
    print("\n── S8: cross-asset — trade ETH book on BTC's move")
    ...
    print("  divergence: BTC moved >=0.15%, ETH's own move < 0.05% (ETH lagging)")
