"""
XRP/USD 1-minute price data from Coinbase. Mirrors eth_data.py exactly,
just hits the XRP-USD product instead of ETH-USD.

Cache stored per day as xrp_cb_YYYYMMDD.json.
"""
...
COINBASE_URL = "https://api.exchange.coinbase.com/products/XRP-USD/candles"
MAX_CANDLES = 300


def fetch_xrp_prices(start_ts_sec, end_ts_sec):
    """Returns a dict mapping minute timestamps (unix seconds, as strings)
    to XRP close price. Fetches 1-min Coinbase candles, caches by day."""
    ...
    while current < end_day:
        ...
        cache_path = os.path.join(CACHE_DIR, f"xrp_cb_{day_str}.json")
...
def _fetch_day(day_start_ts, day_str, day_num, total_days):
    """Fetch one full day of 1-min Coinbase XRP-USD candles."""
    prices = {}
...
# Alias for cross-asset code compatibility
fetch_btc_prices = fetch_xrp_prices


def lookup(prices, ts):
    """Return the most recent XRP price at or before ts."""
...
if __name__ == "__main__":
    now = int(time.time())
    p = fetch_xrp_prices(now - 3600, now)
