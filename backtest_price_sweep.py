"""Sweep MAX_FILL_PRICE thresholds on realistic backtest.

Runs the realistic-fill backtest ONCE per asset, then post-filters bets by
fill price to show the tradeoff between # bets and ROI.
"""
...
print(f"Backtest: {ASSET} | {DAYS} days | MAX_FILL_PRICE sweep")
...
# Collect ALL bets (no price filter), then sweep filter afterwards
def collect(market):
...
print(f"\nCollecting all bets across {len(mkts)} markets...")
all_bets = []
...
print(f"\nAll bets collected: {len(all_bets)}\n")

# Sweep max_price threshold
THRESHOLDS = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.95]
print(f"{'max_fill':>8} {'bets':>6} {'wins':>6} {'wr':>5} {'wagered':>9} {'P&L':>9} {'ROI':>7} {'$/day':>8}")
print("-" * 72)
for thr in THRESHOLDS:
    filtered = [b for b in all_bets if b["fill"] <= thr]
    n = len(filtered)
    if n == 0:
        print(f"{thr:>8.2f}    0      0      0%       $0.00     $0.00     n/a")
        continue
