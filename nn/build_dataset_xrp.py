"""XRP version of build_dataset_eth.py. Same 14-feature schema, same logic,
just hits KXXRP15M Kalshi markets and XRP-USD Coinbase prices.

Output: nn/data/dataset_xrp.npz
"""
...
OUT_PATH = os.path.join(OUT_DIR, "dataset_xrp.npz")
...
def fetch_xrp_markets(days):
    import requests, time
    url = "https://api.elections.kalshi.com/trade-api/v2/markets"
    min_close = int(time.time()) - days * 86400
    cursor = None
    out = []
    print(f"Fetching settled {KALSHI_SERIES_XRP} markets last {days}d...", flush=True)
    for page in range(1000):
        params = {"series_ticker": KALSHI_SERIES_XRP, "status": "settled",
...
def build_window(market, xrp_prices):
    ...
    xrp_t0 = xrp_data.lookup(xrp_prices, t0)
    if xrp_t0 is None:
        return None
    ...
    for m in range(WINDOW_MINUTES):
        t = t0 + m * 60
        xrp = xrp_data.lookup(xrp_prices, t)
        cand = _candle_at(candles, t)
        if xrp is None or cand is None: continue
        yc = float(cand["yes_close"])
        if not (0.01 < yc < 0.99): continue
        xrp_history.append(xrp)
        ret_t0  = (xrp / xrp_t0) - 1.0
        ret_1m  = (xrp / last_xrp) - 1.0 if last_xrp else 0.0
        last_xrp = xrp
        abs_max  = max(abs_max, abs(ret_t0))
        ret_5m = (xrp / xrp_history[-6]) - 1.0 if len(xrp_history) >= 6 else 0.0
...
def main():
    ...
    markets = fetch_xrp_markets(DATA_DAYS)
    if not markets:
    ...
    for mk in markets:
        try:
            ...
            pass
    xrp_prices = xrp_data.fetch_xrp_prices(min(ts_list) - 600, max(ts_list) + 1800)
    print(f"  {len(xrp_prices)} XRP minute prices loaded", flush=True)

    Xs, masks, ys, tss, tickers = [], [], [], [], []
    skipped = 0
    print(f"\nExtracting XRP features per window…", flush=True)
    for i, mk in enumerate(markets):
        r = build_window(mk, xrp_prices)
        if r is None:
            skipped += 1
            continue
        X, mask, y, t0 = r
