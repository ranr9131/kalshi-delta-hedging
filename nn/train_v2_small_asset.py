"""
Train v2_small for any asset. Reads dataset name from ASSET env var.
e.g. ASSET=sol → trains from data/dataset_sol.npz → best_v2_small_sol.pt
"""
...
ASSET = os.environ.get("ASSET", "sol").lower()

ROOT       = os.path.dirname(os.path.abspath(__file__))
DATA_PATH  = os.path.join(ROOT, "data", f"dataset_{ASSET}.npz")
CKPT_DIR   = os.path.join(ROOT, "checkpoints")
CKPT_PATH  = os.path.join(CKPT_DIR, f"best_v2_small_{ASSET}.pt")
STATS_PATH = os.path.join(CKPT_DIR, f"train_stats_v2_small_{ASSET}.json")
...
def main():
    ...
    print(f"Model v2_small ({ASSET}): {n_params:,} parameters\n")
