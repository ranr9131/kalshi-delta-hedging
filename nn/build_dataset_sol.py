"""SOL version of build_dataset_eth.py. Same 14-feature schema, same logic,
just hits KXSOL15M Kalshi markets and SOL-USD Coinbase prices.

Output: nn/data/dataset_sol.npz
"""
...
OUT_PATH = os.path.join(OUT_DIR, "dataset_sol.npz")
...
def fetch_sol_markets(days):
    import requests, time
    url = "https://api.elections.kalshi.com/trade-api/v2/markets"
    min_close = int(time.time()) - days * 86400
    cursor = None
    out = []
    print(f"Fetching settled {KALSHI_SERIES_SOL} markets last {days}d...", flush=True)
    for page in range(1000):
        params = {"series_ticker": KALSHI_SERIES_SOL, "status": "settled",
...
def build_window(market, sol_prices):
    ...
    sol_t0 = sol_data.lookup(sol_prices, t0)
    if sol_t0 is None:
        return None
    ...
    for m in range(WINDOW_MINUTES):
        t = t0 + m * 60
        sol = sol_data.lookup(sol_prices, t)
        cand = _candle_at(candles, t)
        if sol is None or cand is None: continue
        yc = float(cand["yes_close"])
        if not (0.01 < yc < 0.99): continue
        sol_history.append(sol)
        ret_t0  = (sol / sol_t0) - 1.0
        ret_1m  = (sol / last_sol) - 1.0 if last_sol else 0.0
        last_sol = sol
        abs_max  = max(abs_max, abs(ret_t0))
        ret_5m = (sol / sol_history[-6]) - 1.0 if len(sol_history) >= 6 else 0.0
...
def main():
    ...
    markets = fetch_sol_markets(DATA_DAYS)
    if not markets:
    ...
    for mk in markets:
        try:
            ...
            pass
    sol_prices = sol_data.fetch_sol_prices(min(ts_list) - 600, max(ts_list) + 1800)
    print(f"  {len(sol_prices)} SOL minute prices loaded", flush=True)

    Xs, masks, ys, tss, tickers = [], [], [], [], []
    skipped = 0
    print(f"\nExtracting SOL features per window…", flush=True)
    for i, mk in enumerate(markets):
        r = build_window(mk, sol_prices)
        if r is None:
            skipped += 1
            continue
        X, mask, y, t0 = r
