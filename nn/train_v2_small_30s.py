"""Train v2_small NN on 30-second resolution dataset.

Architecture identical to train_v2_small_asset.py but uses 30-step sequences
(30 timesteps × 14 features) instead of 15-step. Reads from dataset_30s_{asset}.npz.

Output: checkpoints/best_v2_small_30s_{asset}.pt
"""
...
DATA_PATH  = os.path.join(ROOT, "data", f"dataset_30s_{ASSET}.npz")
CKPT_DIR   = os.path.join(ROOT, "checkpoints")
CKPT_PATH  = os.path.join(CKPT_DIR, f"best_v2_small_30s_{ASSET}.pt")
STATS_PATH = os.path.join(CKPT_DIR, f"train_stats_v2_small_30s_{ASSET}.json")
...
# 30s resolution: T+4 = step 8, T+13 = step 26
MIN_DEC = 8
MAX_DEC = 26
...
class TruncDataset(Dataset):
    def __init__(self, X, mask, y, random_trunc=True, fixed_dec=20):
...
def main():
    ...
    n_steps = X.shape[1]
    print(f"  Total: {n}  Train: {n_train}  Val: {n_val}  features={n_features}  steps={n_steps}")
    ...
    val_ds   = TruncDataset(Xnorm[n_train:], mask[n_train:], y[n_train:],
                            random_trunc=False, fixed_dec=20)
    ...
    model = TSWinPredictor(
        ...
        max_len=n_steps,  # 30 timesteps (was 15 for v2)
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"Model v2_small_30s ({ASSET}): {n_params:,} parameters\n")
